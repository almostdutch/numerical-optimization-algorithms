...
X0 - initial guess
func_error - anonimous function
"""

import numpy as np
np.random.seed();

def simulated_annealing_algorithm(X0, func_error, options):
    ...
    for iter_no in range(1, N_iter_max + 1):
        ...
        progress_y[0, [iter_no]] = func_error(X);        
        ...
        if (progress_y[0, [iter_no]] < Y_best):
            ...
            toss_a_coin = np.random.rand(); # toss a coin
            temperature = gamma / np.log(iter_no + 2); 
            prob = np.min([1, np.exp(-(progress_y[:, [iter_no]] - Y_best) / temperature)]);
            if (toss_a_coin < prob):
                X_old = progress_x[:, [iter_no]];
