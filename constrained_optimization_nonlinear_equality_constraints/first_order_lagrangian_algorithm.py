...
def first_order_lagrangian_algorithm(X0, Lambda0, func, Dy, Dh, h, options):

    epsilon = 10e-3;
    alpha = 0.01; # step size for X
    beta = 0.01; # step size for Mu
    report = {};
    ...
    X_old = X0;
    Lambda_old = Lambda0;
    W_old = np.concatenate((X0, Lambda0), axis=0);
           
    for iter_no in range(1, N_iter_max + 1):
               
        dX = -(Dy(X_old) + Dh(X_old) @ Lambda_old); # direction for X
        ...
        W = np.concatenate((X,Lambda), axis=0);
        progress_x[:, [iter_no]] = X;
        progress_y[0, [iter_no]] = func(X);

        if (np.linalg.norm(h(X)) < epsilon):
            print('Tolerance in h(X) is reached in %d iterations, exit..' % (iter_no));
            break;
