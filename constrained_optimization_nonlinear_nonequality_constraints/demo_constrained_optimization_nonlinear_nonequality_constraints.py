"""
demo_constrained_optimization_nonlinear_nonequality_constraints.py
...
Update equation for X:
    ...
    d is direction:
        d = - (Df(X_old) + Dh(X_old).T @ Lambda + Dg(X_old).T @ Mu);
...
Update equation for Mu:
    Mu = max{0, Mu_old + gamma * d};
    gamma is step size (<<1);
    d is direction:
        d = g(X_old);
        
If alpha, beta, and gamma are small enough, the algorithm will converge to a fixed point, which satisfies the Lagrange condition.
"""
...
# (Non)linear nonequality constraints:
# g(X) = 15-x1 <= 0;
g = lambda X : 15 - X[0]; 
Dg = lambda X : np.array([[-1], [0]]);

# Initial guess (subject to the equality constraints):
X0 = np.array([[50], [0.4]]);   
Lambda0 = np.array([[0]]);
Mu0 = np.array([[0]]); # Mu0 >= 0
...
start = time.time();
X, report = first_order_lagrangian_algorithm(X0, Lambda0, Mu0, func, Dy, Dh, Dg, h, g, options);
end = time.time();
